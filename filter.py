...
class CustomerFilter(Filter):
    ...
    def apply(self, customers: list[Customer],
        ...
        x = []
        for y in customers:
            if filter_string == str(y.get_id()):
                z = y.get_phone_numbers()
                for a in data:
                    if a.src_number in z or a.dst_number in z:
                        x.append(a)
                return x
        return data
    ...
    def apply(self, customers: list[Customer],
        ...
        try:
            ...
            if filter_string[0] != "L" and filter_string[0] != "G":
                return data
            for i in range(1, len(filter_string)):
                if filter_string[i] not in "0123456789":
                    return data
            y = int(filter_string[1:])
            ...
            for y in x:
                for z in y:
                    if z not in "1234567890.-":
                        return data
            if (float(x[0]) <= -79.697878 or float(x[1]) <= 43.576959 or
                    float(x[2]) >= -79.196382 or float(x[3]) >= 43.799568 or
                    float(x[0]) >= float(x[2]) or float(x[1]) >= float(x[3])):
                return data
            else:
                for a in data:
                    if (float(x[0]) <= a.dst_loc[0] <= float(x[2]) and float(x[1])
                            <= a.dst_loc[1] <= float(x[3]) or float(x[0])
                            <= a.src_loc[0] <= float(x[2]) and float(x[1])
                            <= a.src_loc[1] <= float(x[3])):
                        w.append(a)
                return w
        except IndexError:
            return data
