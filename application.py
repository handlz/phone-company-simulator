...
def process_event_history(log: dict[str, list[dict]],
    ...
    for event_data in log['events']:
        new_bill_date = datetime.datetime.strptime(event_data['time'],
                                                   "%Y-%m-%d %H:%M:%S")
        if billing_month != new_bill_date.month:
            new_month(customer_list, new_bill_date.month, new_bill_date.year)
            billing_month = new_bill_date.month
        if event_data["type"] == "call":
            x = Call(event_data["src_number"], event_data["dst_number"],
                     new_bill_date, event_data["duration"],
                     (event_data["src_loc"][0], event_data["src_loc"][1]),
                     (event_data["dst_loc"][0], event_data["dst_loc"][1]))
            y = find_customer_by_number(event_data["src_number"], customer_list)
            if y is not None:
                y.make_call(x)
            z = find_customer_by_number(event_data["dst_number"], customer_list)
            if z is not None:
                z.receive_call(x)
